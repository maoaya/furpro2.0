...
describe('UsuarioDetalle Facebook Style', () => {
  it('muestra paneles izquierdo y derecho, feed central y controles lógicos', () => {
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: '#222', color: '#FFD700' }}>
          ...
          <main style={{ flex: 1, padding: 32, background: '#222' }}>
            <div style={{ background: '#FFD700', color: '#222', borderRadius: 16, padding: 32, boxShadow: '0 2px 12px #0006', maxWidth: 600, margin: '0 auto' }}>
              <h1>Usuario: Ana Torres</h1>
              <h3>Rol: Admin</h3>
              <p>Email: ana.torres@futpro.com</p>
              <p>Teléfono: +34 600 123 456</p>
              <svg width="100%" height="40" style={{ background: '#222' }}>
                <rect x={0} y={12} width={20} height={28} fill="#FFD700" rx={3} />
                <rect x={22} y={8} width={20} height={32} fill="#FFD700" rx={3} />
                <rect x={44} y={16} width={20} height={24} fill="#FFD700" rx={3} />
              </svg>
    ...
    cy.mount(<UsuarioDetalleMock />);
    cy.contains('Panel de Usuario').should('exist');
    cy.contains('Usuario: Ana Torres').should('exist');
