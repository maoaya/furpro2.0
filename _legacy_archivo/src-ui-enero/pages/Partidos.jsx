...
export default function Partidos() {
  ...
  const [form, setForm] = useState({ nombre: '', fecha: '', lugar: '' });
  const [editId, setEditId] = useState(null);
  const [loading, setLoading] = useState(false);
  const [feedback, setFeedback] = useState('');

  useEffect(() => {
    PartidoService.getPartidos().then(setPartidos);
  }, []);

  const handleOpenForm = (partido = null) => {
    if (partido) {
      setForm({ nombre: partido.nombre || '', fecha: partido.fecha || '', lugar: partido.lugar || '' });
      setEditId(partido.id);
    } else {
      setForm({ nombre: '', fecha: '', lugar: '' });
      setEditId(null);
    }
    setShowForm(true);
    setFeedback('');
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (editId) {
        await PartidoService.updatePartido(editId, form);
        setFeedback('Partido actualizado');
      } else {
        await PartidoService.createPartido(form);
        setFeedback('Partido creado');
      }
      const nuevos = await PartidoService.getPartidos();
      setPartidos(nuevos);
      handleCloseForm();
    } catch (err) {
  ...
  const handleDelete = async id => {
    ...
    try {
      ...
      setPartidos(await PartidoService.getPartidos());
  ...
  return (
    <div style={{ background: '#181818', minHeight: '100vh', color: '#FFD700', padding: 48, borderRadius: '18px', boxShadow: '0 2px 12px #FFD70044', maxWidth: '900px', margin: 'auto' }}>
      ...
      <Button style={{ background: '#FFD700', color: '#181818', border: 'none', borderRadius: 8, padding: '12px 32px', fontWeight: 'bold', fontSize: 18, boxShadow: '0 2px 8px #FFD70088', cursor: 'pointer', marginBottom: 24, transition: 'background 0.3s, color 0.3s' }} onClick={() => handleOpenForm()}>Programar Partido</Button>
      ...
      <ul style={{ fontSize: 18 }}>
        {partidos.map(p => (
          <li key={p.id} style={{ background: '#232323', borderRadius: 8, padding: 16, color: '#FFD700', boxShadow: '0 2px 8px #FFD70022', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <span style={{ fontWeight: 'bold', fontSize: 20 }}>{p.nombre || p.titulo || `Partido ${p.id}`}</span> — Fecha: {p.fecha} — Lugar: {p.lugar}
            </div>
            <div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: '#181818cc', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <form onSubmit={handleSubmit} style={{ background: '#232323', borderRadius: 16, padding: 32, minWidth: 320, boxShadow: '0 2px 16px #FFD70044', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <h3 style={{ color: '#FFD700', fontWeight: 'bold', fontSize: 24, marginBottom: 12 }}>{editId ? 'Editar Partido' : 'Programar Partido'}</h3>
            ...
            <input name="fecha" value={form.fecha} onChange={handleChange} placeholder="Fecha" style={{ padding: 12, borderRadius: 8, border: '1px solid #FFD700', fontSize: 18, marginBottom: 8 }} />
            <input name="lugar" value={form.lugar} onChange={handleChange} placeholder="Lugar" style={{ padding: 12, borderRadius: 8, border: '1px solid #FFD700', fontSize: 18, marginBottom: 8 }} />
