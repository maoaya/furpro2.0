...
export default function SupabaseClientJS() {
  const [actividad] = useState([2, 5, 3, 1, 4]);
  const [msg, setMsg] = useState('');

  function handleConsultar() {
    setMsg('Consultando supabase...');
    setTimeout(() => setMsg(''), 2000);
  }
  function handleFiltrar() {
    setMsg('Filtrando supabase...');
    setTimeout(() => setMsg(''), 2000);
  }
  ...
  // Debug log para inicialización de Supabase
  console.log('[DEBUG] Inicializando Supabase Client...');

  // Ejemplo de gráfico de actividad (solo visual, no funcional en JS puro)
  function mostrarGraficoActividadSupabase() {
    console.log('[DEBUG] Mostrando gráfico de actividad Supabase...');
    // Simulación de datos
    const actividad = [2, 4, 3, 5, 1];
    // Renderizar SVG en consola (solo para debug)
    console.log('[SVG] <svg width="400" height="120">' + actividad.map((val, i) => `<rect x='${i*70+20}' y='${120-val*20}' width='40' height='${val*20}' fill='#FFD700' rx='8'/>`).join('') + '<line x1="20" y1="10" x2="20" y2="110" stroke="#FFD700" stroke-width="2"/><line x1="20" y1="110" x2="380" y2="110" stroke="#FFD700" stroke-width="2"/></svg>');
  }
  mostrarGraficoActividadSupabase();
  ...
  return (
    <div style={{ background: '#181818', minHeight: '100vh', color: '#FFD700', padding: 48, borderRadius: '18px', boxShadow: '0 2px 12px #FFD70044', maxWidth: '700px', margin: 'auto' }}>
      <h2 style={{ fontSize: 32, fontWeight: 'bold', marginBottom: 24 }}>Supabase Client</h2>
      ...
      <div style={{ background: '#232323', borderRadius: 12, padding: 32, marginBottom: 32 }}>
        <h3 style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 18 }}>Gráfico de actividad de supabase</h3>
