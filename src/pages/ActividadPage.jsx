...
export default function ActividadPage() {
  ...
  React.useEffect(() => {
    setLoading(true);
    supabase.from('actividad').select('*').then(({ data, error }) => {
      if (error) setError(error.message);
      else setActividad(data);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: black, color: gold }}>
      <aside style={{ width: 220, background: black, borderRight: `2px solid ${gold}`, padding: 24 }}>
        <h2 style={{ color: gold }}>Actividad</h2>
        <Button onClick={() => window.location.href = '/actividad'}>Actualizar</Button>
      </aside>
      <main style={{ flex: 1, padding: 32, background: black }}>
        <div style={{ background: gold, color: black, borderRadius: 16, padding: 32, boxShadow: '0 2px 12px #0006', maxWidth: 600, margin: '0 auto' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {actividad.map(a => (
                <tr key={a.id} style={{ background: a.id % 2 === 0 ? '#333' : black }}>
                  <td style={{ padding: 10 }}>{a.usuario}</td>
                  <td style={{ padding: 10 }}>{a.accion}</td>
                  <td style={{ padding: 10 }}>{a.fecha}</td>
                </tr>
              ))}
            </tbody>
          </table>
