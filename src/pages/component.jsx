...
const ComponentPage = () => {
  ...
  const [actividad, setActividad] = useState([3, 1, 4, 2, 5]);
  ...
  const componentesFiltrados = componentes.filter(c => c.nombre?.toLowerCase().includes(filtro.toLowerCase()));

  return (
    <div style={{ display: 'flex', background: black, minHeight: '100vh', color: gold }}>
      ...
      <aside style={{ width: 220, background: '#232323', padding: 32, borderRight: '2px solid #FFD700', minHeight: '100vh' }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, color: gold }}>
          ...
          <li style={{ marginBottom: 18 }}><button style={{ background: 'none', color: gold, border: 'none', fontWeight: 'bold', fontSize: 18, cursor: 'pointer' }}>Historial</button></li>
        </ul>
      </aside>
      ...
      <main style={{ flex: 1, padding: 48, maxWidth: 800, margin: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: 16, marginBottom: 32 }}>
          <input type="text" placeholder="Filtrar por nombre..." value={filtro} onChange={e => setFiltro(e.target.value)} style={{ padding: 8, borderRadius: 8, border: '1px solid #FFD700', background: '#232323', color: gold, width: '40%' }} />
          <button onClick={consultar} style={{ background: gold, color: '#181818', border: 'none', borderRadius: 8, padding: '10px 24px', fontWeight: 'bold', boxShadow: '0 2px 8px #FFD70088', cursor: 'pointer' }}>Consultar</button>
          <button onClick={actualizar} style={{ background: '#232323', color: gold, border: '1px solid #FFD700', borderRadius: 8, padding: '10px 24px', fontWeight: 'bold', cursor: 'pointer' }}>Actualizar</button>
          <button onClick={verHistorial} style={{ background: '#232323', color: gold, border: '1px solid #FFD700', borderRadius: 8, padding: '10px 24px', fontWeight: 'bold', cursor: 'pointer' }}>Ver historial</button>
        </div>
        {/* Feed de componentes */}
        <section style={{ background: '#232323', borderRadius: 12, padding: 32, marginBottom: 32, boxShadow: '0 2px 12px #FFD70022' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            <li style={{ marginBottom: 18, color: gold }}>Componente consultado: principal</li>
            <li style={{ marginBottom: 18, color: gold }}>Historial consultado: FutPro Stars</li>
          </ul>
        </section>
      ...
      <aside style={{ width: 220, background: '#232323', padding: 32, borderLeft: '2px solid #FFD700', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <button style={{ background: gold, color: '#181818', border: 'none', borderRadius: 8, padding: '12px 32px', fontWeight: 'bold', fontSize: 18, marginBottom: 18, boxShadow: '0 2px 8px #FFD70088', cursor: 'pointer' }}>Consultar componente</button>
        <button style={{ background: '#232323', color: gold, border: '1px solid #FFD700', borderRadius: 8, padding: '12px 32px', fontWeight: 'bold', fontSize: 18, marginBottom: 18, cursor: 'pointer' }}>Ver historial</button>
      </aside>
    </div>
  );
};
