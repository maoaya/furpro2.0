...
const EditarEquipoPage = () => {
  ...
  const handleSubmit = e => {
    ...
    const ranking = form.ranking.value.trim();
    let errs = {};
    if (!nombre) errs.nombre = 'El nombre del equipo es obligatorio.';
    if (!ranking || isNaN(ranking) || ranking < 1) errs.ranking = 'Ranking debe ser un número mayor a 0.';
    setErrores(errs);
  ...
  return (
    <div style={{ background: '#181818', minHeight: '100vh', color: '#FFD700', padding: 48, borderRadius: '18px', boxShadow: '0 2px 12px #FFD70044', maxWidth: '700px', margin: 'auto' }}>
      ...
      <form style={{ maxWidth: '100%', margin: '0 auto' }} onSubmit={handleSubmit} noValidate>
        ...
        <label style={{ display: 'block', marginBottom: 18, fontWeight: 'bold', fontSize: 18 }}>
          Ranking:
          <input name="ranking" type="number" style={{ width: '100%', padding: 12, borderRadius: 8, marginTop: 6, fontSize: 18, border: '1px solid #FFD700', background: '#232323', color: '#FFD700' }} />
          {errores.ranking && <span style={{ color: '#FF4D4F', fontSize: 15 }}>{errores.ranking}</span>}
        </label>
      ...
      <div style={{ background: '#232323', borderRadius: 12, padding: 32, marginBottom: 32 }}>
        <h3 style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 18 }}>Gráfico de actividad de equipo</h3>
...
describe('EditarEquipoPage', () => {
  it('renderiza formulario y botones principales', () => {
    render(<EditarEquipoPage />);
    expect(screen.getByText(/Equipo actualizado|Filtrando datos de equipo|Guardar|Filtrar|Actualizar/i)).toBeDefined;
  });
});
