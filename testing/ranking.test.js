...
describe('API Health Check Tests', () => {
  ...
  describe('GET /api/ranking/jugadores', () => {
    ...
    test('debería filtrar por edad máxima', async () => {
      const res = await request(app)
        .get('/api/ranking/jugadores?edadMax=25')
        .expect(200);
      ...
      // Verificar que todos los jugadores tienen edad <= 25
      res.body.ranking.forEach(jugador => {
        expect(jugador.edad).toBeLessThanOrEqual(25);
      });
    });

    test('debería filtrar por rango de edad', async () => {
      const res = await request(app)
        .get('/api/ranking/jugadores?edadMin=22&edadMax=28')
        .expect(200);

      expect(res.body.status).toBe('ok');
      
      // Verificar que todos los jugadores están en el rango
      res.body.ranking.forEach(jugador => {
        expect(jugador.edad).toBeGreaterThanOrEqual(22);
        expect(jugador.edad).toBeLessThanOrEqual(28);
      });
    });
    ...
    test('debería filtrar por equipoId', async () => {
      const res = await request(app)
        .get('/api/ranking/jugadores?equipoId=1')
        .expect(200);

      expect(res.body.status).toBe('ok');
      
      // Si hay resultados, verificar que el ID coincide
  ...
  describe('GET /api/ranking/campeonatos', () => {
    ...
    test('debería filtrar por categoría', async () => {
      const res = await request(app)
        .get('/api/ranking/campeonatos?categoria=Sub-20')
        .expect(200);

      expect(res.body.status).toBe('ok');
      
      // Verificar que las categorías contienen el texto buscado
      res.body.ranking.forEach(campeonato => {
        expect(campeonato.categoria.toLowerCase()).toContain('sub-20');
      });
    });

    test('debería filtrar por fecha inicio desde', async () => {
      const res = await request(app)
        .get('/api/ranking/campeonatos?fechaInicioDesde=2025-03-01')
        .expect(200);

      expect(res.body.status).toBe('ok');
      
      // Verificar que todas las fechas son >= a la fecha filtro
      res.body.ranking.forEach(campeonato => {
        expect(new Date(campeonato.fecha_inicio)).toBeInstanceOf(Date);
        expect(new Date(campeonato.fecha_inicio).getTime()).toBeGreaterThanOrEqual(
          new Date('2025-03-01').getTime()
        );
      });
    });

    test('debería filtrar por fecha inicio hasta', async () => {
      const res = await request(app)
        .get('/api/ranking/campeonatos?fechaInicioHasta=2025-06-30')
        .expect(200);
      ...
      // Verificar que todas las fechas son <= a la fecha filtro
      res.body.ranking.forEach(campeonato => {
        expect(new Date(campeonato.fecha_inicio).getTime()).toBeLessThanOrEqual(
  ...
  it('debe mostrar el ranking por género', async () => {
    const res = await request(app)
      ...
      // este endpoint es simulado en entorno de test principal, no requiere token en server-test
      ;
